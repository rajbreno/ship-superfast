...
A final call-to-action ("Ready to ship?") and a `Footer` component (`components/navigation/footer.tsx`) with the copyright, theme toggle, and links to Privacy Policy and Terms of Service pages. The footer uses a responsive `flex-col sm:flex-row` layout. The Terms and Privacy pages also include the `<Footer />` component.
...
All page sections use `max-w-7xl` for consistent content width.
...
| `components/navigation/footer.tsx` | Footer with copyright, theme toggle, and legal links |
